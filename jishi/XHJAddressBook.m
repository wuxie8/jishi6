...
-(NSMutableArray*)getAllPerson
{
    ...
    for (NSInteger i = 0; i < nPeople; i++)
    {
        ...
        if (personimageData==NULL) {
            personimage=[UIImage imageNamed:@"DefaultAvatar"];
        }
        else
        {
            personimage=[UIImage imageWithData:personimageData];
        }
        addressBook.image=personimage;
        if (personimageData)
            
            CFRelease((__bridge CFTypeRef)(personimageData));
                ...
                switch (j) {
                    case 0: {// Phone number
                        ...
                        NSCharacterSet *setToRemove = [[ NSCharacterSet characterSetWithCharactersInString:@"0123456789"]
                                                       invertedSet ];
                       addressBook.tel = [[addressBook.tel componentsSeparatedByCharactersInSet:setToRemove] componentsJoinedByString:@""];
        ...
        if ([UtilTools isBlankString:addressBook.name1]) {
            addressBook.name1=addressBook.tel;
            addressBook.phonename=addressBook.tel;
