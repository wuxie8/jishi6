...
- (NSURLSessionDataTask *)getNoTipJSONHomeShopping:(NSString *)name
                                    parameters:(NSDictionary *)parameters
                                       success:(void (^)(NSURLSessionDataTask *task, id responseObject))success
                                       failure:(void (^)(NSURLSessionDataTask *task, NSError *error))failure {
    if ([NetWorkUtil currentNetWorkStatus] == NET_UNKNOWN) {
    ...
    return [self GET:url parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        BOOL IsSuccess=[responseObject boolForKey:@"IsSuccess"];
        if (IsSuccess) {
            success(task, responseObject);
...
- (NSURLSessionDataTask *)postNoTipJSONHomeShopping:(NSString *)name
                                     parameters:(NSDictionary *)parameters
                                        success:(void (^)(NSURLSessionDataTask *task, id responseObject))success
                                        failure:(void (^)(NSURLSessionDataTask *task, NSError *error))failure
{
